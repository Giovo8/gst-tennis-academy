    ...
    <section id="programmi" className="max-w-7xl mx-auto px-6 sm:px-8 py-12 sm:py-16 md:py-20">
      <div className="space-y-10 sm:space-y-12">
        <div className="text-center mb-10 sm:mb-12">
          <p className="text-xs sm:text-sm uppercase tracking-[0.2em] font-bold text-cyan-600 mb-4">
            Corsi e Abbonamenti
          </p>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent mb-3 sm:mb-4">
      ...
      <div className="flex flex-col gap-6 sm:gap-8">
        ...
        if (section.layout_type === "single_box") {
          ...
          return (
            <div key={section.id} className="group rounded-xl sm:rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 md:p-10 hover:border-cyan-300 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
              <div className="flex flex-col gap-6 sm:gap-8 md:flex-row md:items-center md:justify-between">
                <div className="flex-1">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 group-hover:text-cyan-600 transition-colors">{section.section_title}</h3>
                ...
                <div className="text-left md:text-right shrink-0">
                  <span className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">{item.price}€</span>
        ...
        if (section.layout_type === "frequency_grid") {
          return (
            <div key={section.id} className="rounded-xl sm:rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 md:p-10 hover:border-cyan-200 transition-all shadow-sm">
              <div className="mb-5 sm:mb-7">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">{section.section_title}</h3>
                {section.section_description && (
                  <p className="text-base text-gray-600 leading-relaxed">{section.section_description}</p>
                )}
              </div>
              ...
              <div className="grid gap-5 sm:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                {section.items.map((item, idx) => (
                  <div key={idx} className="group rounded-lg sm:rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-5 sm:p-7 hover:bg-white hover:border-cyan-300 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <p className="text-xs sm:text-sm uppercase tracking-wider text-cyan-600 mb-3 sm:mb-4 font-bold">{item.frequency}</p>
                    <div className="space-y-2 sm:space-y-3">
                      {item.price_monthly && (
                        <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">
                          {item.price_monthly}€<span className="text-xs sm:text-sm font-normal text-gray-500"> /mese</span>
                        </p>
                      )}
                      {item.price_yearly && (
                        <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">
                          {item.price_yearly}€<span className="text-xs sm:text-sm font-normal text-gray-500"> /anno</span>
                        </p>
                      )}
                    </div>
                  </div>
