...
export default function AtletaVideosAdminView() {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="text-xs font-semibold text-secondary/60 uppercase tracking-wider mb-1">
            LIBRERIA VIDEO (Solo Visualizzazione)
          </div>
          <h1 className="text-3xl font-bold text-secondary mb-2">
            Video Lezioni
          </h1>
          <p className="text-secondary/70 font-medium">
            Visualizza i video disponibili per gli atleti
          </p>
        </div>
        <button
          onClick={() => loadVideos()}
      ...
      {filteredVideos.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600">
            {videos.length === 0
              ? "Nessun video disponibile per la visualizzazione."
        ...
        <div className="space-y-3">
          {filteredVideos.map((video) => {
            ...
            return (
              <Link
                key={video.id}
                href={`/dashboard/admin/video-lessons/${video.id}`}
