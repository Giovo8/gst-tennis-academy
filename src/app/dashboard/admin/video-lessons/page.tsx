...
export default function VideoLessonsPage() {
  ...
  async function createVideo() {
    ...
    try {
      ...
      // Send notification if video is assigned to specific user
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-secondary/70 font-medium">
            Assegna video lezioni personalizzati agli atleti
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {(showCreateForm || editingVideo) && (
        <div className="bg-white rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">Titolo *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">URL Video * (YouTube, Vimeo)</label>
              <input
                type="url"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-secondary mb-2">Descrizione</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">URL Thumbnail</label>
              <input
                type="url"
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">Durata (minuti)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">Categoria</label>
              <select
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">Livello</label>
              <select
            ...
            <div>
              <label className="block text-sm font-semibold text-secondary mb-2">Assegna a Utente</label>
              <select
          ...
          <div className="mt-6 flex gap-3">
            {editingVideo ? (
              <>
                <button
                  onClick={updateVideo}
                  className="px-6 py-2.5 text-sm font-medium text-white bg-secondary rounded-md hover:opacity-90 transition-all flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="px-6 py-2.5 text-sm font-medium text-secondary bg-white rounded-md hover:bg-secondary/5 transition-all"
                >
                  Annulla
                </button>
              </>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-4 text-secondary/70">Caricamento video...</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filteredVideos.map((video) => (
            <div
              key={video.id}
              className="bg-white rounded-xl p-5 hover:bg-secondary/5 transition-all"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4 min-w-0 flex-1">
                  {video.thumbnail_url && (
                    <div className="w-32 h-20 rounded-lg overflow-hidden bg-secondary/5 flex-shrink-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {video.description && (
                      <p className="text-sm text-secondary/70 mt-1 line-clamp-2">{video.description}</p>
                    )}
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      <span className="px-2 py-1 rounded-md text-xs font-bold bg-secondary text-white">
                      ...
                      {video.duration_minutes && (
                        <span className="text-xs text-secondary/70">{video.duration_minutes} min</span>
                      )}
                      {video.assigned_user && (
          ...
          {filteredVideos.length === 0 && (
            <div className="text-center py-12">
              <Video className="h-12 w-12 text-secondary/40 mx-auto mb-3" />
              <p className="text-secondary/70">Nessun video trovato</p>
            </div>
          )}
        </div>
      )}
