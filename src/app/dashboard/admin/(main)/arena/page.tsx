...
export default function AdminArenaPage() {
  ...
  const getLevelColor = (level: string) => {
    switch (level.toLowerCase()) {
      ...
      case "argento":
        return "bg-gray-100 text-gray-700 border border-black/10";
      case "bronzo":
          ...
          <button
            ...
            className={`p-2.5 rounded-lg transition-all ${
              activeTab === "info"
                ...
                : "text-secondary/70 bg-white border border-black/10 hover:bg-secondary hover:text-white"
            }`}
            title="Info"
          >
                ...
                return (
                  <li key={`${entry.userId}-${index}`}>
                    <button
                      ...
                      className="w-full flex items-center gap-4 py-3 px-3 rounded-lg hover:opacity-90 transition-opacity text-left"
